...
public class StackSort {
    ...
    VectorStack<Integer> lower = new VectorStack<Integer>(n+1);
    VectorStack<Integer> upper = new VectorStack<Integer>(n+1);
    
    for(int i=0;i < n;i++) {
    	int a = data[i];
    	while(lower.isEmpty() == false && lower.peek() > a) {
    		ShiftToRight(lower,upper);
    	}
    	while(upper.isEmpty() == false && lower.isEmpty() == false && upper.peek() < a) {
    		ShiftToLeft(lower,upper);
    	}
    	lower.push(a);
    }
    while(lower.isEmpty() == false) {
    	ShiftToRight(lower,upper);
    }
    for(int i=0;i<n;i++) {
    	result[i]=upper.pop();
    ...
    private static void ShiftToLeft(VectorStack<Integer> lower, VectorStack<Integer> upper) {
    	if( upper.isEmpty() == true)
    		return;
    	lower.push(upper.pop());
    }
    private static void ShiftToRight(VectorStack<Integer> lower, VectorStack<Integer> upper) {
    	if( lower.isEmpty() == true)
    		return;
